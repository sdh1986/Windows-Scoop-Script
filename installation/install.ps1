...
# --- Start: Auto-Detect Fastest Proxy ---
$GHPROXY = 'https://gh-proxy.com' # Default Fallback

try {
    Write-Host "Checking for get-fastestgiturl.ps1 to optimize download speed..." -ForegroundColor Cyan
    if (Test-Path "$PSScriptRoot\get-fastestgiturl.ps1") {
        . "$PSScriptRoot\get-fastestgiturl.ps1" -ErrorAction Stop
        
        # NOTE: If Git is NOT installed yet, Get-FastestGitUrl might return null or print a warning.
        # This is expected for fresh installs. The script will safely fallback to the default.
        ...
        if (-not [string]::IsNullOrWhiteSpace($FastestGitUrl)) {
            ...
            Write-Host "Speed test returned no result (Git might be missing). Using default: $GHPROXY" -ForegroundColor Yellow
        }
    } else {
        Write-Warning "get-fastestgiturl.ps1 not found in current directory. Using default proxy."
    }
}
catch {
    Write-Warning "Error running proxy detection: $_. Using default proxy."
}
# --- End: Auto-Detect Fastest Proxy ---

# TODO: Use a specific version of Scoop and the main bucket
$SCOOP_PACKAGE_REPO = "$GHPROXY/https://github.com/ScoopInstaller/Scoop/archive/master.zip"
$SCOOP_MAIN_BUCKET_REPO = "$GHPROXY/https://github.com/ScoopInstaller/Main/archive/master.zip"

$SCOOP_PACKAGE_GIT_REPO = "$GHPROXY/https://github.com/ScoopInstaller/Scoop"
$SCOOP_MAIN_BUCKET_GIT_REPO = "$GHPROXY/https://github.com/ScoopInstaller/Main"
